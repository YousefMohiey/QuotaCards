...
// Desktop Windows build shares this UI: platform-only pieces are removed
// from its index.html, and the guards below keep the shared code safe.
const IS_DESKTOP = document.body.dataset.platform === "desktop";
...
// Desktop engine is Standard only; the other segments are phone-only.
if (IS_DESKTOP) transport = "vless";
...
function appDisplay(a) {
  if (lang === "ar" && a && (AR_APP_NAMES[a.pkg] || AR_DESKTOP_NAMES[a.pkg])) return AR_APP_NAMES[a.pkg] || AR_DESKTOP_NAMES[a.pkg];
  return (a && (a.label || a.pkg)) || "";
}
// Arabic display names for the curated Windows processes (same idea as the
// phone map above, keyed by exe name).
const AR_DESKTOP_NAMES = {
  "chrome.exe": "كروم",
  "msedge.exe": "إيدج",
  "firefox.exe": "فايرفوكس",
  "Discord.exe": "ديسكورد",
  "Steam.exe": "ستيم",
  "RiotClientServices.exe": "رايوت",
  "VALORANT-Win64-Shipping.exe": "فالورانت",
  "EpicGamesLauncher.exe": "إيبك",
  "Battle.net.exe": "باتل نت",
  "Spotify.exe": "سبوتيفاي",
  "Telegram.exe": "تيليجرام",
  "WhatsApp.exe": "واتساب",
  "qbittorrent.exe": "كيوبت تورنت",
  "Code.exe": "في إس كود",
};
...
// Phone-only VPN settings button (removed from the desktop page).
...
async function paintBg() {
  // Phone-only toggle: no battery screen exists on desktop.
...
// Phone-only settings buttons (removed from the desktop page).
document.addEventListener("visibilitychange", () => { if (!document.hidden && !IS_DESKTOP) paintBg(); });
if (!IS_DESKTOP) paintBg();
